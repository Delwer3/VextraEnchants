...
public class CategoryFive {
    ...
    public static List<Inventory> getCategoryFiveInv(EdgeEnchants plugin, Player player) {
        ...
        for (int i = 0; i < plugin.getEnchants().getEnchantlist().size(); i++){
            ...
            List<String> categorylist = plugin.getEnchants().getCategory5List();
    ...
    public static void openMenuCategoryFive(EdgeEnchants plugin,Player player, Integer number){
        List<Inventory> invMenu = getCategoryFiveInv(plugin, player);
        if (invMenu == null || invMenu.isEmpty()){
            return;
        }
        player.setMetadata("OpenMenuEnchant5", new FixedMetadataValue(plugin, invMenu));
        if (number != null){
            number = number;
        }else{
            number = 0;
