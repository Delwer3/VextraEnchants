...
public class CategorySix {
    public static List<Inventory> getCategorySixInv(EdgeEnchants plugin, Player player){
        ...
        for (int i = 0; i < plugin.getEnchants().getEnchantlist().size(); i++){
            ...
            List<String> categorylist = plugin.getEnchants().getCategory6List();
    ...
    public static void openMenuCategorySix(EdgeEnchants plugin,Player player, Integer number){
        List<Inventory> invMenu = getCategorySixInv(plugin, player);
        if (invMenu == null || invMenu.isEmpty()){
            return;
        }
        player.setMetadata("OpenMenuEnchant6", new FixedMetadataValue(plugin, invMenu));
        if (number != null){
            number = number;
        }else{
            number = 0;
