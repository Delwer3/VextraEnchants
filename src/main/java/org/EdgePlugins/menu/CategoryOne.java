...
public class CategoryOne {
    public static List<Inventory> getCategoryOneInv(EdgeEnchants plugin, Player player){
        ...
        for (int i = 0; i < plugin.getEnchants().getEnchantlist().size(); i++){
            ...
            List<String> categorylist = plugin.getEnchants().getCategory1List();
    ...
    public static Inventory createNewInv(Player player, Integer integer){
        integer = integer +1;
        Inventory inv = Bukkit.createInventory(player, 9 * 3, Message.getTranslate("&7Page "+integer.toString()));
        return inv;
    }

    public static void openMenuCategoryOne(EdgeEnchants plugin,Player player, Integer number){
        List<Inventory> invMenu = getCategoryOneInv(plugin, player);
        if (invMenu == null || invMenu.isEmpty()){
            return;
        }
        player.setMetadata("OpenMenuEnchant1", new FixedMetadataValue(plugin, invMenu));
        if (number != null){
            number = number;
        }else{
