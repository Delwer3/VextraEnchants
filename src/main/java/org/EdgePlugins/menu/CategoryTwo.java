...
public class CategoryTwo {
    public static List<Inventory> getCategoryTwoInv(EdgeEnchants plugin, Player player){
        ...
        for (int i = 0; i < plugin.getEnchants().getEnchantlist().size(); i++){
            ...
            List<String> categorylist = plugin.getEnchants().getCategory2List();
    ...
    public static void openMenuCategoryTwo(EdgeEnchants plugin,Player player, Integer number){
        List<Inventory> invMenu = getCategoryTwoInv(plugin, player);
        if (invMenu == null || invMenu.isEmpty()){
            return;
        }
        player.setMetadata("OpenMenuEnchant2", new FixedMetadataValue(plugin, invMenu));
        if (number != null){
            number = number;
        }else{
            number = 0;
