...
public class CategoryThree {
    public static List<Inventory> getCategoryThreeInv(EdgeEnchants plugin, Player player){
        ...
        for (int i = 0; i < plugin.getEnchants().getEnchantlist().size(); i++){
            ...
            List<String> categorylist = plugin.getEnchants().getCategory3List();
    ...
    public static void openMenuCategoryThree(EdgeEnchants plugin,Player player, Integer number){
        List<Inventory> invMenu = getCategoryThreeInv(plugin, player);
        if (invMenu == null || invMenu.isEmpty()){
            return;
        }
        player.setMetadata("OpenMenuEnchant3", new FixedMetadataValue(plugin, invMenu));
        if (number != null){
            number = number;
        }else{
            number = 0;
